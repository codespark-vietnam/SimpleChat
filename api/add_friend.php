...
try {
    // 1. Cập nhật trạng thái 'pending' -> 'accepted'
    $stmt = $conn->prepare(
        "UPDATE friendships 
         SET status = 'accepted' 
    ...
    if ($stmt->rowCount() > 0) {
        // 2. Lấy thông tin của người bạn mới (người gửi)
        $stmt_user = $conn->prepare("SELECT user_id, display_name FROM users WHERE user_id = ?");
        $stmt_user->execute([$sender_id]);
        $new_friend = $stmt_user->fetch();

        echo json_encode([
            'status' => 'success', 
            'message' => 'Kết bạn thành công.',
            'new_friend' => $new_friend // Trả về thông tin bạn mới
        ]);
    } else {
